		...
		\subsection{Callable Range Accruals}
	 	...
	 	\subsection{Exercise value}
		 	...
		 	$\begin{array}{lcl}
			 	...
			 	=payoff * \sum_{k=1}^{i}\frac{\sum_{j=k - 1}^{j=k}\indicator (S_{t_j} \in [S_{min},S_{max}])}{365} * e^{-r(T_i - T_k)} -  \sum_{{T_k}^{'} \in {{T_1}^{'}, {T_p}^{'}}} ({T_f}^{'} {\delta_f}^{'} e^{-r({T_i} - {T_k}^{'})})
		 	\end{array}$
	 	...
	 	$\begin{array}{lcl}
		 	H(T_i) = \E(E(T_{i}) * e^{r(\tau-T_i)} + \sum_{t \in[T_{i+1}; \tau ]}E(t)*e^{-r(t - T_i)} -  \sum_{t \in[{T_{i+1}}^{'}; \tau]} ({T_f}^{'} {\delta_f}^{'} e^{-r({T_i} - {T_k}^{'})})|S_{T_i})\\
